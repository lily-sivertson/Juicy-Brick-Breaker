...
func _on_Ball_body_entered(body):
	if body.has_method("hit"):
		...
		tween.tween_property($Images/Highlight, "modulate:a", 0.0, .4)
		$Images/Highlight.scale = Vector2(2.0,2.0)
		tween.tween_property($Images/Highlight, "scale", Vector2(1.0,1.0), .3).set_trans(Tween.TRANS_BOUNCE).set_ease(Tween.EASE_IN)
